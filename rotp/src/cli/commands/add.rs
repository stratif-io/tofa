...
pub fn run(args: AddArgs, vault_path: PathBuf) -> CliResult {
    ...
    if let Some(secret) = &args.secret {
        let name = args
            .name
            .ok_or("--name is required when using --secret")?;
        return add_single(&name, secret, &mut vault, &vault_path, &pass);
    }
