...
#[test]
fn freeotp_npm_entry_secret_decoded() {
    let bytes = fixture_bytes("freeotp-fake.json");
...
#[test]
fn freeotp_heroku_entry_has_non_default_params() {
    let bytes = fixture_bytes("freeotp-fake.json");
    let entries = parse_json_bytes(&bytes).unwrap();
    let e = entries
        .iter()
        .find(|e| e.meta.issuer.as_deref() == Some("Heroku"))
        .expect("Heroku entry should be present");
