...
fn migration_qr_totp_sha1_account() {
    ...
    let acc = accounts.iter().find(|a| {
        a.meta.issuer.as_deref() == Some("Demo TOTP SHA1")
...
fn migration_qr_8digit_account() {
    let accounts = parse_migration(&scan_migration_uri()).unwrap();
    let acc = accounts.iter().find(|a| {
        a.meta.issuer.as_deref() == Some("Demo TOTP 8-digit")
...
fn migration_and_standard_qr_same_secret() {
    ...
    let accounts = parse_migration(&scan_migration_uri()).unwrap();
    let migration = accounts.iter().find(|a| {
        a.meta.issuer.as_deref() == Some("Demo TOTP SHA1")
    }).unwrap();
    ...
    assert_eq!(code_standard, code_migration, "both fixtures use the same secret");
}
