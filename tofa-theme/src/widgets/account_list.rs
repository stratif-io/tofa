...
impl AccountListState {
    pub fn new() -> Self { Self { list_state: ListState::default() } }
    pub fn select(&mut self, i: usize) { self.list_state.select(Some(i)); }
    pub fn selected(&self) -> Option<usize> { self.list_state.selected() }
}
...
impl AccountList {
    pub fn new(entries: Vec<AccountEntry>) -> Self {
        Self { entries, theme: ThemeMode::Dark }
    }
    ...
    fn render(self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
        ...
        let items: Vec<ListItem> = self.entries.iter().enumerate().map(|(i, e)| {
            ...
            let line = Line::from(vec![
                Span::styled(
                    format!("{:<14}", e.issuer),
                    Style::default().fg(if is_selected { self.theme.text() } else { muted }),
                ),
                Span::styled(code_str, Style::default().fg(code_color).add_modifier(Modifier::BOLD)),
            ]);
            ListItem::new(line)
        }).collect();

        let list = List::new(items)
            .highlight_style(Style::default().bg(self.theme.surface()).fg(brand));
...
mod tests {
    ...
    fn make_entries() -> Vec<AccountEntry> {
        vec![
            AccountEntry { issuer: "GitHub".into(), account: "u@ex.com".into(), code: Some("847 392".into()), seconds: 20 },
            AccountEntry { issuer: "AWS".into(),    account: "root".into(),     code: Some("128 045".into()), seconds: 15 },
        ]
    }
