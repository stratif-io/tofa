...
pub fn render(f: &mut Frame, area: Rect, state: &AppState) {
    ...
    let idx = state.otpauth_list_index.min(total - 1);
    let qr_lines = &state.otpauth_list_qrs[idx];
    let title = state
        .otpauth_list_titles
        .get(idx)
        .cloned()
    ...
    // Required: QR + border (2) + title (1) + counter (1) + blank (1) + help (1)
    let needed_w = qr_w.max(40) + 4;
    let needed_h = qr_h + 5;
    ...
    let qr_paragraph: Vec<Line> = qr_lines
        .iter()
    ...
    f.render_widget(Paragraph::new(qr_paragraph), qr_rect);
...
fn render_too_small(f: &mut Frame, area: Rect, needed_w: u16, needed_h: u16) {
    ...
    f.render_widget(
        Paragraph::new(Line::from(Span::styled(
            format!("Need at least {needed_w}×{needed_h} — resize and retry"),
            Style::default().fg(theme::TEXT_MUTED),
        )))
        .alignment(Alignment::Center),
        rows[2],
