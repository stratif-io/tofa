...
fn import_json_roundtrip() {
    ...
    Command::cargo_bin("tofa").unwrap()
        .env("TOFA_PASSPHRASE", "testpass")
        .env("TOFA_VAULT", tmp_a.path().join("vault.enc").to_str().unwrap())
        .arg("init").assert().success();
    Command::cargo_bin("tofa").unwrap()
        .env("TOFA_PASSPHRASE", "testpass")
        .env("TOFA_VAULT", tmp_a.path().join("vault.enc").to_str().unwrap())
    ...
    let tmp_b = setup();
    tofa(&tmp_b).args(["import", export_file.to_str().unwrap()])
...
fn import_skips_duplicates() {
    let tmp = setup();
    tofa(&tmp).args(["add", "--name", "GitHub:carlo", "--secret", "JBSWY3DPEHPK3PXP"])
        .assert().success();
    ...
    tofa(&tmp).args(["import", f.to_str().unwrap()])
