...
fn remove_confirms_and_deletes() {
    let tmp = setup();
    tofa(&tmp).args(["remove", "GitHub:carlo"]).write_stdin("y\n")
...
fn remove_aborts_on_n() {
    let tmp = setup();
    tofa(&tmp).args(["remove", "GitHub:carlo"]).write_stdin("n\n")
        .assert().success().stderr(contains("Aborted"));
    tofa(&tmp).arg("list").assert().success().stdout(contains("GitHub:carlo"));
}

#[test]
fn remove_not_found() {
    let tmp = setup();
    tofa(&tmp).args(["remove", "nope"]).write_stdin("y\n")
